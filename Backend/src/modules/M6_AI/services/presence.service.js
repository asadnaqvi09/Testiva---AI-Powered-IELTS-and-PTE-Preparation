...
export const markUserOnline = async (userId, preference = "General") => {
  const pref = normalizeTopicTag(preference);
...
export const markUserOffline = async (userId, preference = null) => {
  const pref = preference ? normalizeTopicTag(preference) : await redis.get(`${PRESENCE_KEY}${userId}`);
...
export const getOnlineCount = async (preference = null) => {
  if (preference) {
    return await redis.scard(`${ROOM_KEY}${normalizeTopicTag(preference)}`);
  }
...
export const getRoomOnlineUsers = async (preference) => {
  return await redis.smembers(`${ROOM_KEY}${normalizeTopicTag(preference)}`);
};
