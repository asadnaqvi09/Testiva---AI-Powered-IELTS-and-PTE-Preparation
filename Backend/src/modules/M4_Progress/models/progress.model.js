...
export const startNewAttempt = async (data, client = pool) => {
    ...
    try {
        ...
        if (existing.rows.length > 0) {
            ...
            throw new Error(
                "Test already in progress. Resume or abandon existing attempt.",
            );
        }
...
export const saveUserResponse = async (data, client = pool) => {
    ...
    try {
        ...
        if (qType === "mcq" || qType === "true_false" || qType === "yes_no") {
...
export const finalizeAttempt = async (id, data, client = pool) => {
    const { overall_band_score, feedback, client_completed_at, status = 'completed' } = data;
    const dbClient = client === pool ? await pool.connect() : client;
    try {
        if (client === pool) await dbClient.query("BEGIN");
        const result = await dbClient.query(
            `UPDATE test_attempts 
             SET overall_band_score = $1, feedback = $2, client_completed_at = $3, status = $4,
             server_synced_at = NOW(), updated_at = NOW()
             WHERE id = $5 RETURNING id, user_id, status`,
