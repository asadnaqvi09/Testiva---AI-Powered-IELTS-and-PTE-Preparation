import Joi from "joi";
import { normalizeTopicTag, TOPIC_TAGS } from "../../../utils/topicTag.js";
...
export const validateCreatePost = (data) =>
  validate(
    Joi.object({
      topic_tag: Joi.string().required().custom((value, helpers) => {
        const normalized = normalizeTopicTag(value);
        if (!TOPIC_TAGS.includes(normalized)) {
          return helpers.error("any.invalid");
        }
        return normalized;
      }).messages({
        "any.invalid": `topic_tag must be one of: ${TOPIC_TAGS.join(", ")}`,
      }),
...
export const validateGetPosts = (query) => {
  ...
  if (normalized.topic_tag && normalized.topic_tag !== "ALL") {
    normalized.topic_tag = normalizeTopicTag(normalized.topic_tag);
  }
  return validate(
    Joi.object({
      topic_tag: Joi.string()
        .valid("ALL", ...TOPIC_TAGS)
        .default("ALL")
        .messages({
